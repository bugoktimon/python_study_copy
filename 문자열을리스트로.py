...
step2 = step1.split() # split을 쓰면 리스트가 된다. (기억하기) / split은 문자열을 리스트로. 조인은 스트링을 문자열로.
print(step2, type(step2)) # 리스트가됨
...
test = "안녕, 나는 파이썬 공부에 열중하고 있어... 근데 너는"


words = [ 
        
        3. word.strip() #공백을 또 제거해
        1. for word in str(text).replace(',', ' ').split() # 쉼표를 공백으로 떨어뜨려 => [안녕, 나는, 파이썬, 공부에, 열중하고, 있어..., 근데, 너는] => for문 8번 진행 
        2. if word.strip() # 공백이 있다면 제거하고 

]

["안녕", "나는", "파이썬", "공부에", "열중하고", "있어...", "근데", "너는"]

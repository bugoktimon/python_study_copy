...
import pandas as pd # 값을 xy좌표 같이 가져감 행 전체를 날려버리는
...
df_region['상세지역'] = df_region['통합 분류1'].apply(
    lambda x: x.split('-')[-1].strip() if '-' in str(x) else x.strip() # apply()안에 1회성 함수 안에 들어가는데 lambda x로 기재. 아래에서는 get clean~ 함수로 기재
)
# 위, 공백제거 strip() -> ex, 학원   입니다. 위 의미는 -기준으로 나눈다. -는 자연히 없어짐. 
...
# 빈문자열이라도 삽입
all_region_names = list(df_region['상세지역'].dropna().unique()) # 데이터전처리



def get_top_10_keywords(series):   # 시리즈는 키 값, 키 값
...
def get_clean_keywords(text):
    ...
    for word in words:
        ...
        if any(region in word for region in all_region_names): continue # if 문이 실행되면 건너뜀. 실행되지 않아야만 clean.words로 들어갈 수 있음
...
def merge_and_rank(series):
    ...
    return [item[0] for item in Counter(merged_list).most_common(10)] # 글자 중복으로 몇개씩 나왔는지. 가장 많이 나온 10순위만 걸러내고(내림차순), 
# 예시
# [("민원", 2),("치료", 10), .....] (내림차순) => 치료, 민원 (가장많이 나온 키워드 순 10개)
...
visual_df = df_region.groupby('상세지역').agg(    # 뭉치고 
    뉴스건수=('상세지역', 'count'),                # 강원에 몇개 기사가 있었는지
    키워드순=('정제된_리스트', merge_and_rank)      # 강원지역에 10위권 키우드가 나오게 되는 것임
...
visual_df['상위3개키워드'] = visual_df['키워드순'].apply(lambda x: ', '.join(x[:3])) # 앞에서 세번째 까지 0 1 2, join은 문자열 만듬. 범례에 넣으려고.
# 상위3개키워드 컬럼 추가하고 키워드순 리스트로 값이 되어있는데 하나의 문자열로 쉽표가 포함된 3자리까지 합쳐서 넣어주기 (join)

plt.figure(figsize=(14, 7))


ax = sns.barplot( # seaborn sns. barplot은 한 막대기 나오는 것.
    data=visual_df.reset_index(), # 키 제일 큰애가 앞으로 데꼬와라
    x='상세지역', 
    y='뉴스건수', 
    hue='상위3개키워드', # 범례
...
for p in ax.patches:
    if p.get_height() > 0: # 0건 이상인 경우만 표시
        ax.annotate(
            f"{int(p.get_height()):,}",     # 천자리에 쉼표 들어가게. [1. 표시할 텍스트]: 막대 높이(뉴스건수)를 정수로 바꾼 뒤, 3자리마다 콤마(,)를 넣음 (예: 1,250)
